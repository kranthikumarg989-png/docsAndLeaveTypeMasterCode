...
Partial Public Class Operatorappraisal
    ...
    Private Sub GetAppraisalData(ByVal rno As String)
        ...
        Dim draa1 As DataRow
        dsw12 = GetEmp(rno)
        If dsw12.Tables(0).Rows.Count <> 0 Then
            draa1 = dsw12.Tables(0).Rows(0)
            lblemp.Text = draa1("empcode").ToString
        ...
        Dim con As New SqlConnection(constr)
        con.Open()
        cmd1 = New SqlCommand("select max(CONVERT(varchar(10), dateofappraisal, 101)) as dt from app_operatorappraisal where empcode = '" & lblemp.Text & "' and status = 'S'", con)
        ...
        con.Open()
        cmd1 = New SqlCommand("select * from app_operatorappraisal where empcode = '" & rno & "' and (status <> 'AD' or status <>'EA')", con)
        ...
        While (dra1.Read())
            ...
            End If
            txtcomments.Text = dra1("remarks").ToString
    ...
    Protected Sub txtbutton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtbutton.Click
        ...
        Try
            MyGlobal.Open_Con()
            MyGlobal.Con_Str()
            Call MyGlobal.dbSp_open("Hrmis_insOptappraisal")

            Cmd.Parameters.AddWithValue("@empcode", lblemp.Text.Trim)
            Cmd.Parameters.AddWithValue("@purposeofappraisal", rdpurpose.SelectedValue.Trim)
            Cmd.Parameters.AddWithValue("@status", "AS")
            ...
            Cmd.Parameters.AddWithValue("@senddh", "N")
            Cmd.Parameters.AddWithValue("@remarks", txtcomments.Text)
            Cmd.Parameters.AddWithValue("@ratedbyempno", Session("empcode"))
