...
Partial Public Class SelfAppraisal
    ...
    Dim empcode
    Private Sub SelfAppraisal_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        MyGlobal.Open_Con()
        MyGlobal.Con_Str()
        MyApp.GetfiscalYear()
    ...
    Protected Sub getDetails()
        ...
        If recstatus = True Then
            ...
            If dsdata.Tables(0).Rows.Count > 0 Then
                ...
                lblsect.Text = dr1("sectioncode").ToString
                lbldept.Text = dr1("departmentcode").ToString
                lbldesig.Text = dr1("designation").ToString
                lblstatus.Text = dr1("emptype").ToString
                If Not dr1("foreignemp").ToString Is System.DBNull.Value Then
                    Session("foreigner") = dr1("foreignemp").ToString
                Else
                    Session("foreigner") = "N"
                End If
            Else
        ...
        Dim dr2 As DataRow
        Dim sqltext
        sqltext = "select * from app_charactersetting  where currentyear='Y' and setby='empno' and accesscode='" & lblempcode.Text.Trim & "'"
        ...
        Dim dsmc As DataSet
        Dim drmc As DataRow
        dsmc = GetMedical(lblempcode.Text.Trim)
        ...
        dsabs = GetAbscent(lblempcode.Text.Trim)
        ...
        dslate = GetLAte(lblempcode.Text.Trim)
        ...
        dscou = GetcOUNSELLING(lblempcode.Text.Trim)
        ...
        dsexp = GetExplanation(lblempcode.Text.Trim)
        ...
        Dim dsv As DataSet
        Dim drv As DataRow
        dsv = Getverbal(lblempcode.Text.Trim)
        ...
        Dim dsw As DataSet
        Dim drw As DataRow
        dsw = Getwritten(lblempcode.Text.Trim)
        ...
        Dim dsf As DataSet
        Dim drf As DataRow
        dsf = Getfirst(lblempcode.Text.Trim)
        ...
        Dim dsfi As DataSet
        Dim drfi As DataRow
        dsfi = Getfinal(lblempcode.Text.Trim)
        ...
        Dim dsfs As DataSet
        Dim drfs As DataRow
        dsfs = Getsuspension(lblempcode.Text.Trim)
        ...
        Dim dssc As DataSet
        Dim drsc As DataRow
        dssc = Getshowcase(lblempcode.Text.Trim)
        ...
        Dim dsr As DataSet
        Dim drr As DataRow
        dsr = GetMisRpt(lblempcode.Text.Trim)
    ...
    Protected Sub chkDynamic_CheckedChanged(ByVal Sender As Object, ByVal e As System.EventArgs)
        ...
        Dim lbl As Label = DirectCast(GrdPerformance.FooterRow.FindControl("Lblbpts"), Label)
        If lbl.Text = "" Then
            lbl.Text = 0
        End If
    ...
    Protected Sub calculateTotal(ByVal Sender As Object, ByVal e As System.EventArgs)
        ...
        ''' TO PERFORM TOTAL CALUCLATION IF WEIGHT IS CHANGED
        ...
        For i As Integer = 0 To grdJg.Rows.Count - 1
            Dim passno As String = grdJg.Rows(i).Cells(0).Text
            Dim grade As String = DirectCast(grdJg.Rows(i).FindControl("rdprating"), RadioButtonList).SelectedValue
            Dim weights As String = DirectCast(grdJg.Rows(i).FindControl("pweight"), TextBox).Text
            Dim point As Label = DirectCast(grdJg.Rows(i).FindControl("ppoint"), Label)
            Dim grade1 As RadioButtonList = DirectCast(grdJg.Rows(i).FindControl("rdprating"), RadioButtonList)

            If weights = "" Or weights.Length = "0" Then
                MessageBox("your weightage should be greater than 0")
                DirectCast(grdJg.Rows(i).FindControl("pweight"), TextBox).Focus()
                Exit Sub
            End If
            ...
            End If
            totpts = Double.Parse(point.Text.ToString)
            totals = Double.Parse(totals)
            totals += totpts

            Dim x = grade1.SelectedValue
            If (x = "" Or weights = "0") Then
                x = 0
            End If
            tgrade = Double.Parse(x)
            totgrade = Double.Parse(totgrade)
            ...
            DirectCast(grdJg.Rows(i).FindControl("rdprating"), RadioButtonList).Focus()
        Next
        DirectCast(grdJg.FooterRow.FindControl("lblpts"), Label).Text = totals
        DirectCast(grdJg.FooterRow.FindControl("Lblmarks"), Label).Text = totgrade
        Dim lbl As Label = DirectCast(GrdPerformance.FooterRow.FindControl("Lblbpts"), Label)
        If lbl.Text = "" Then
            lbl.Text = 0
        End If
        lblfinpoints.Text = totals + lbl.Text
        calculateGrade(lblfinpoints.Text)
    End Sub
    Protected Sub chkDynamic2_CheckedChanged(ByVal Sender As Object, ByVal e As System.EventArgs)
        ...
        Dim lbl As Label = DirectCast(grdJg.FooterRow.FindControl("Lblpts"), Label)
        If lbl.Text = "" Then
            lbl.Text = 0
        End If
        lblfinpoints.Text = total + lbl.Text
    ...
    Protected Sub calculateTotal2(ByVal Sender As Object, ByVal e As System.EventArgs)
        ...
        For i As Integer = 0 To GrdPerformance.Rows.Count - 1
            Dim passno As String = GrdPerformance.Rows(i).Cells(0).Text
            Dim grade As String = DirectCast(GrdPerformance.Rows(i).FindControl("rdbutton"), RadioButtonList).SelectedValue
            Dim weightS As String = DirectCast(GrdPerformance.Rows(i).FindControl("txtbweit"), TextBox).Text
            Dim point As Label = DirectCast(GrdPerformance.Rows(i).FindControl("lblbpoints"), Label)
            Dim grade1 As RadioButtonList = DirectCast(GrdPerformance.Rows(i).FindControl("rdbutton"), RadioButtonList)
            ...
            DirectCast(GrdPerformance.Rows(i).FindControl("rdbutton"), RadioButtonList).Focus()
        Next
        DirectCast(GrdPerformance.FooterRow.FindControl("Lblbpts"), Label).Text = totalS
        Dim lbl As Label = DirectCast(grdJg.FooterRow.FindControl("Lblpts"), Label)
        If lbl.Text = "" Then
            lbl.Text = 0
        End If
    ...
    Private Sub calculateGrade(ByVal pts As Decimal)
        ...
        ElseIf pts <= 0 Then
            lblfingrade.Text = "E"
        ElseIf pts > 0 And pts < 51 Then
            lblfingrade.Text = "E"
        ElseIf pts > 50 And pts < 61 Then
            lblfingrade.Text = "D"
        ElseIf pts > 60 And pts < 71 Then
            lblfingrade.Text = "C"
        ElseIf pts > 70 And pts < 86 Then
            lblfingrade.Text = "B"
        ElseIf pts > 85 And pts < 101 Then
            lblfingrade.Text = "A"
        End If
    End Sub
    ...
    Protected Sub txtbutton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtbutton.Click
        ...
        Dim fd As Date
        fd = CDate(fd1)
        If rdpurpose.SelectedValue = "1/2 Yearly" And ddlhalf.SelectedValue = "-" Then
            MessageBox("Pls. select the purpose of appraisal")
            Exit Sub
        End If

        '''' to confirm the weightage doesnot exceed/ below the criteria set

        Dim weight1 As TextBox
        Dim total1 As Double
        Dim txtVal1 As Double
        For i As Integer = 0 To grdJg.Rows.Count - 1
            weight1 = DirectCast(grdJg.Rows(i).FindControl("pweight"), TextBox)
            If weight1.Text = "" Then
                weight1.Text = 0
            End If
            txtVal1 = Double.Parse(weight1.Text.ToString)
            total1 = Double.Parse(total1)
            total1 += txtVal1

        Next
        If total1 <> 80 Then
            MessageBox("weightage must be 80% for section1 ")
            Exit Sub
        End If

        '''

        Dim weight2 As TextBox
        Dim total2 As Double
        Dim txtVal2 As Double
        For i As Integer = 0 To GrdPerformance.Rows.Count - 1
            weight2 = DirectCast(GrdPerformance.Rows(i).FindControl("txtbweit"), TextBox)
            If weight2.Text = "" Then
                weight2.Text = 0
            End If
            txtVal2 = Double.Parse(weight2.Text.ToString)
            total2 = Double.Parse(total2)
            total2 += txtVal2
        Next
        If total2 <> 20 Then
            MessageBox("weightage must be 20% for Performance character/behaviour measurement ")
            Exit Sub
        End If
        '''
        ...
        Try
            MyGlobal.Open_Con()
            MyGlobal.Con_Str()
            Call MyGlobal.dbSp_open("hrmis_insAppraisal_self")

            Cmd.Parameters.AddWithValue("@empcode", lblempcode.Text.Trim)
            ...
            If Session("foreigner") = "Y" Then
                ...
                If dsemp.Tables(0).Rows.Count <> 0 Then
                    ...
                    Call MyGlobal.dbSp_open("insert_appraisal_selfcontractexpiry")
            ...
            ElseIf Session("foreigner") = "N" Then
                ...
                While (dra.Read())
                    ...
                    Dim dsp As DataSet
                    Dim drp As DataRow
                    dsp = GetProbation(lblempcode.Text.Trim)
                ...
                End While
                fstat = "P"
                Call MyGlobal.dbSp_open("insert_appraisal")
                Cmd.Parameters.AddWithValue("@empcode", lblempcode.Text.Trim)
                Cmd.Parameters.AddWithValue("@appraisal", "Y")
                Cmd.Parameters.AddWithValue("@status", "S")
    ...
    Function GetEmp1() As DataSet
        ...
        con.Open()
        Dim Command As New SqlCommand
        Command = New SqlCommand("select * from empmaster where emptype = 'Contract'  and resigned = 'N' and nationality <> 'JAP' and empcode ='" & lblempcode.Text.Trim & "' order by empcode", con)
        Command.CommandType = CommandType.Text
        Dim DataAdap = New SqlDataAdapter(Command)
        DataAdap.Fill(ds, "emp")
        con.Close()
