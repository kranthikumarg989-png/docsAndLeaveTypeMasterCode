...
Partial Public Class ER_letter_IssuePage
    ...
    Private Sub CreateDynamicTable(ByVal Rows As Integer, ByVal columns As Integer)
        Dim ds1 As DataSet
        Dim dr1 As DataRow
        Dim Lfields1, Lfieldtype, lfielddetail
        Dim labellist As New ArrayList
        Dim labellist2 As New ArrayList

        Dim lb As New ListBox
        Dim strtext, prefix
        Dim o, k
        strtext = "select field_name ,fielddetail,ltrim(rtrim(field_type)) as field_type,Ltrim(rtrim(prefix)) as prefix  from HRMIS_ER_LETTER_WIZARD where lettername='" & DropDownList1.SelectedItem.Text & "' and status =1"
        ds1 = GetLetterFields(strtext)
        If ds1.Tables(0).Rows.Count <> 0 Then
            o = ds1.Tables(0).Rows.Count
            For k = 0 To o - 1
                dr1 = ds1.Tables(0).Rows(k)
                Lfields1 = dr1("field_name").ToString
                lfielddetail = dr1("fielddetail").ToString
                Lfieldtype = dr1("field_type").ToString
                lb.Items.Add(dr1("field_type").ToString)
                prefix = dr1("prefix").ToString
                labellist.Add(Lfields1)
                labellist2.Add(lfielddetail)
            Next
        End If
        ...
        For q As Integer = 0 To tblRows - 1
            ...
            For j As Integer = 0 To tblCols - 1
                ...
                ElseIf j = 1 Then
                    ...
                    val = val.Remove(0, prefix.Length)
                    txtBox.ID = val
                    If lb.Items(q).Text = "Text Area" Then
                        txtBox.TextMode = TextBoxMode.MultiLine
                    End If
                    tc.Controls.Add(txtBox)
                    tr.Cells.Add(tc)
                    If val.ToString.Contains("date") = True Or lb.Items(q).Text = "Text Date" Then
                        Dim calen As New AjaxControlToolkit.CalendarExtender
                        calen.ID = "cal" & val
                        calen.TargetControlID = val
                        calen.Format = "MM/dd/yy"
                        tc.Controls.Add(calen)
                        tr.Cells.Add(tc)
                    End If
                End If
            Next j
    ...
    Protected Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        ...
        If HtmlEditor1.Text.Trim <> "" Then
            ...
            SqlDataSource3.InsertParameters.Add("issuedate", DateTime.Now)
            SqlDataSource3.InsertParameters.Add("createdby", Session("empcode").ToString)
    ...
    Protected Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        'Dim strcontent, keysign As String
        ...
        If dsabs.Tables(0).Rows.Count <> 0 Then
            ...
            For m = 0 To l - 1
                ...
                If status = 1 Then
                    Dim txtt As TextBox
                    txtt = DirectCast(Me.PlaceHolder1.FindControl(newvalue), TextBox)
                    If newvalue.ToString.Contains("date") = True Then
                        Dim fd1 As String
                        fd1 = txtt.Text.Trim
                        Dim strdate() As String = fd1.Split("/"c)
                        fd1 = strdate(1) & "/" & strdate(0) & "/" & strdate(2)
                        txt = strtexts.Replace(Lfields, fd1)
                        strtexts = txt
                    Else
                        txt = strtexts.Replace(Lfields, txtt.Text)
                        strtexts = txt
    ...
    Protected Sub DropDownList1_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DropDownList1.SelectedIndexChanged
        ...
        If ds1.Tables(0).Rows.Count <> 0 Then
            ...
            Next
        End If
        If DropDownList1.SelectedItem.Text = "Preliminarycounseling" Then
    ...
    Sub sign()
        Dim cmd As New SqlCommand
        Dim str, find1, sig, des, dep As String
        Dim con As New SqlConnection(constr)
        con.Open()
        ...
        'Else
        If DropDownList1.Text = "Employeecounseling" Or DropDownList1.Text = "Preliminarycounseling" Then
        Else
            find1 = DirectCast(Me.PlaceHolder1.FindControl("keyinempnoforsignature"), TextBox).Text
            If find1 <> "" Then
                str = "select empname,designation,department.departmentcode,department.departmentname from empmaster,department where empmaster.departmentcode = department.departmentcode and empcode='" & find1 & "' "
                cmd = New SqlCommand(str, con)
                dr = cmd.ExecuteReader
                While dr.Read
                    sig = dr(0).ToString
            ...
            If HtmlEditor1.Text.Contains(find1) = True And HtmlEditor1.Text.Contains(n) = False Then
                ...
                sig = sig + Environment.NewLine + des + Environment.NewLine + dep
                Dim sig2 = sig.Replace(Environment.NewLine, "<br/>")

                HtmlEditor1.Text = HtmlEditor1.Text.Replace(find1, sig2)
            ElseIf HtmlEditor1.Text.Contains(n) = True Then
                sig = "DATUK MANIMARAN ANTHONY"
                des = "CHIEF EXECUTIVE OFFICER"
                sig = sig + Environment.NewLine + des + Environment.NewLine + dep

                Dim sig2 = sig.Replace(Environment.NewLine, "<br/>")
                HtmlEditor1.Text = HtmlEditor1.Text.Replace(n, sig2)
            End If
            'HtmlEditor1.Text = find1 + Environment.NewLine + desdep
            'HtmlEditor1.Text = HtmlEditor1.Text + Environment.NewLine + desdep
        End If
    End Sub
    Sub ddlsel()
        Dim find1, find2, find3, find4, find5, find6, find7, find8, find9, find10, crby
        Dim dt = Date.Now.ToString
        crby = Session("empcode")
        Dim cmd As New SqlCommand
        MyGlobal.Con_Str()
        Dim con As New SqlConnection(constr)
        MyGlobal.Open_Con()
        ...
        If DropDownList1.Text = "Directfinalwarning" Then
            ...
            cmd.Parameters.AddWithValue("@itimedate", "")
            cmd.Parameters.AddWithValue("@vwdate", "")
            cmd.Parameters.AddWithValue("@fwdate", "")
            cmd.Parameters.AddWithValue("@desc", find1)
            cmd.Parameters.AddWithValue("@secode", find2)
            cmd.Parameters.AddWithValue("@ldate", find3)
            cmd.Parameters.AddWithValue("@cby", crby)
        ...
        ElseIf DropDownList1.Text = "Explanationletter" Then
            ...
            cmd.Parameters.AddWithValue("@crtby", crby)
            cmd.Parameters.AddWithValue("@edi", "")
            cmd.Parameters.AddWithValue("@revi", "")
        ...
        ElseIf DropDownList1.Text = "Finalwarninghostel" Then
            ...
            cmd.Parameters.AddWithValue("@latetime", "")
            cmd.Parameters.AddWithValue("@latedt", "")
            cmd.Parameters.AddWithValue("@ldate", find1)
        ...
        ElseIf DropDownList1.Text = "Finalwarningmorethan1hour" Then
            ...
            cmd.ExecuteNonQuery()
            MessageBox("Record saved to er_finalwarningless30")
            'insert into er_finalwarningmore1 values(@ecode,@latecomingdate,@latecomingtime,@letterdate,@firstwarningdate,@lettergivenby,getdate(),@keyinby)
        ...
        ElseIf DropDownList1.Text = "Finalwarningletter" Then
            ...
            find2 = DirectCast(Me.PlaceHolder1.FindControl("keyinempnoforsignature"), TextBox).Text
            find3 = DirectCast(Me.PlaceHolder1.FindControl("performanceperiod"), TextBox).Text
            'find4 = DirectCast(Me.PlaceHolder1.FindControl("vwdate"), TextBox).Text
        ...
        ElseIf DropDownList1.Text = "Firstwarning" Then
            ...
            find4 = DirectCast(Me.PlaceHolder1.FindControl("counselingdate"), TextBox).Text
            find5 = DirectCast(Me.PlaceHolder1.FindControl("keyinempnoforsignature"), TextBox).Text
            cmd.Parameters.AddWithValue("@ecode", empcode.Text)
            cmd.Parameters.AddWithValue("@itimedate", find4)
            cmd.Parameters.AddWithValue("@wdate", "")
        ...
        ElseIf DropDownList1.Text = "Formaladvice" Then
            ...
            cmd.Parameters.AddWithValue("@creatby", crby)
            cmd.Parameters.AddWithValue("@edit", "")
        ...
        ElseIf DropDownList1.Text = "Preliminarycounseling" Then
            ...
            cmd.Parameters.AddWithValue("@revision", "")
            cmd.Parameters.AddWithValue("@cont", HtmlEditor1.Text)
            cmd.Parameters.AddWithValue("@createdby", crby)
            cmd.ExecuteNonQuery()
            MessageBox("Record saved to er_preliminarycounseling")
            Session("ERpcemp") = empcode.Text
            Session("ERpcltdt") = find8
            Response.Redirect("Preliminarycounselling.aspx")
        ...
        ElseIf DropDownList1.Text = "Verbalwarningmorethan1hour" Then
            ...
            cmd.Parameters.AddWithValue("@latecomingdate", find3)
            cmd.ExecuteNonQuery()
            MessageBox("Record saved to er_verbalwarningmorethan1hour")
