...
Partial Public Class IncrementFor_NonPFP
    ...
    Protected Sub Incpfpsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Incpfpsave.Click
        Dim ft As String
        ...
        If recstatus = True Then
            If dsdata.Tables(0).Rows.Count > 0 Then
                Try
                    ...
                    con.Open()
                    Cmd = New SqlCommand("update hr_increment set cbasicsalary = '" & bsa.Text & "', incamount = '" & splinc.Text & "', nbasicsalary = '" & nbsa.Text & "', poallowance = '" & pall.Text & "', vdate = '" & ft & "',edate = '" & td & "' where empcode='" & empno.Text & "'", con)
                    Cmd.ExecuteNonQuery()
            ...
            Else
                ...
                Try
                    ...
                    Dim con As New SqlConnection(constr)
                    con.Open()
                    Cmd = New SqlCommand("insert into hr_increment (empcode,cbasicsalary,incamount,nbasicsalary,poallowance,vdate,edate) values('" & empno.Text & "','" & bsa.Text & "','" & splinc.Text & "','" & nbsa.Text & "','" & pall.Text & "','" & ft & "','" & td & "')", con)
                    Cmd.ExecuteNonQuery()
                Catch ex As SqlException
                    labelmsg.Text = ex.Message
                    labelmsg.ForeColor = Drawing.Color.Green
        ...
        Response.Redirect("IncrementNonPFPLetter.aspx")
    End Sub
End Class