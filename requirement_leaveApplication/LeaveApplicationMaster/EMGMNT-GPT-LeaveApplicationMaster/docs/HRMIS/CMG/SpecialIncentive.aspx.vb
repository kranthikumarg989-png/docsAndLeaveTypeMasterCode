...
Partial Public Class SpecialIncentive
    ...
    Protected Sub IncSPLsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles IncSPLsave.Click
        Dim ft As String
        ...
        ad = CDate(ad1)
        Session("letdt") = ltdt.Text.Trim


        Dim bd1 As String
        Dim bd As String

        bd1 = Date.Now.ToShortDateString
        ...
        If recstatus = True Then
            If dsdata.Tables(0).Rows.Count > 0 Then
                Try
                    ...
                    con.Open()
                    Cmd = New SqlCommand("update hr_increment set cbasicsalary = '" & bsa.Text & "', incamount = '" & splinc.Text & "', nbasicsalary = '" & nbsa.Text & "', poallowance = '" & pall.Text & "', pfpallowance = '" & pfpall.Text & "',oldpallowance = '" & oldpall.Text & "',vdate = '" & ft & "',edate = '" & td & "' where empcode='" & empno.Text & "'", con)
                    Cmd.ExecuteNonQuery()
            ...
            Else
                ...
                Try
                    ...
                    Dim con As New SqlConnection(constr)
                    con.Open()
                    Cmd = New SqlCommand("insert into hr_increment (empcode,cbasicsalary,incamount,nbasicsalary,poallowance,oldpallowance,pfpallowance,vdate,edate) values('" & empno.Text & "','" & bsa.Text & "','" & splinc.Text & "','" & nbsa.Text & "','" & pall.Text & "','" & oldpall.Text & "','" & pfpall.Text & "','" & ft & "','" & td & "')", con)
                    Cmd.ExecuteNonQuery()
                Catch ex As SqlException
                    labelmsg.Text = ex.Message
                    labelmsg.ForeColor = Drawing.Color.Green
        ...
        Response.Redirect("SplIncenctiveLetter.aspx")
    End Sub
End Class