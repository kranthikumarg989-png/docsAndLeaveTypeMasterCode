...
Partial Public Class ProbTrialPeriodExt
    ...
    Protected Sub Savetrialext_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Savetrialext.Click
        ...
        If recstatus = True Then
            If dsdata.Tables(0).Rows.Count > 0 Then
                Try
                    ...
                    conn.Open()
                    Cmd = New SqlCommand("update emp_probationextend set vdate='" & dd & "',months='" & months.Text & "',improveareas='" & probperiod.Text & "' where empcode='" & empno.Text & "'", conn)
                    Cmd.ExecuteNonQuery()
            ...
            Else
                Try
                    ...
                    Dim conn As New SqlConnection(constr)
                    conn.Open()
                    Cmd = New SqlCommand("insert into emp_probationextend (empcode,vdate,months,improveareas) values('" & empno.Text & "','" & dd & "','" & months.Text & "','" & probperiod.Text & "')", conn)
                    Cmd.ExecuteNonQuery()
        ...
        Response.Redirect("ProbTrialExtRpt.aspx")
    End Sub
End Class