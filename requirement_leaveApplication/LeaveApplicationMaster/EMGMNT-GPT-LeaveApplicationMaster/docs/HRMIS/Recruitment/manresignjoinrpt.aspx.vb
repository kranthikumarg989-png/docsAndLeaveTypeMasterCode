...
Partial Public Class manresignjoinrpt
    ...
    Protected Sub save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles save.Click
        If empcode.Text <> "" Then
            Session("final") = "{empmaster.Empcode} =  '" & empcode.Text & "' And {empmaster.resigned} =  'N'"
        Else
            ...
            End If
            If rb1.SelectedValue = "dept" Then
                Session("final") = "{empmaster.dateofjoin} >= datetime('" & (fromdt.Text) & "')  And {empmaster.dateofjoin} <= datetime('" & (todt.Text) & "') and {empmaster.departmentcode} =  '" & (Val(dept.Text.Trim)) & "'"
            ElseIf rb1.SelectedValue = "desig" And ddlcat.SelectedValue <> "A" Then
                Session("final") = "{empmaster.dateofjoin} >= datetime('" & (fromdt.Text) & "')  And {empmaster.dateofjoin} <= datetime('" & (todt.Text) & "') and {empmaster.designation} =  '" & (desig.Text) & "' and {empmaster.foreignemp} = '" & (Trim(ddlcat.SelectedValue)) & "'"
            ElseIf rb1.SelectedValue = "desig" And ddlcat.SelectedValue = "A" Then
                Session("final") = "{empmaster.dateofjoin} >= datetime('" & (fromdt.Text) & "')  And {empmaster.dateofjoin} <= datetime('" & (todt.Text) & "') and {empmaster.designation} =  '" & (desig.Text) & "'"
            ElseIf rb1.SelectedValue = "sect" Then
                Session("final") = "{empmaster.dateofjoin} >= datetime('" & (fromdt.Text) & "')  And {empmaster.dateofjoin} <= datetime('" & (todt.Text) & "') and {empmaster.sectioncode} =  '" & (Val(section.Text.Trim)) & "'"
            ElseIf rb1.SelectedValue = "A" Then
                Session("final") = "{empmaster.dateofjoin} >= datetime('" & (fromdt.Text) & "')  And {empmaster.dateofjoin} <= datetime('" & (todt.Text) & "')"
            End If
        End If
        Response.Redirect("manresignjoinrptviewer.aspx")
    End Sub
